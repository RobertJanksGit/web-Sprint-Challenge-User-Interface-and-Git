...
Semantic HTML is HTML that that describes its meaning by its name. This helps the browser know what’s it looking at, and helps us know as well.

2. Name two big differences between ```display: block;``` and ```display: inline;```.

display: block; will cause the element to display along the cross axis whereas display: inline; causes the element to display along the main axis.
...
It allows developers to commit multiple versions of their code. Allowing them to go back to earlier versions if needed. It also allows several developers work on separate parts of the code and push them all to the same file.
